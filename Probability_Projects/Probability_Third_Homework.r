...
get_prob2_data()
# тут я попереносив числа бо інакше вони не читались коли були в 1 рядок :(
...
# P(A*C) = 1/2 * 1/2 = 1/4 = P(A)P(C)
# P(B*C) = 1/2 * 1/2 = 1/4 = P(B)P(C)
# A,B,C are pairwaise independent
# P(A,B,C)=0 because odd + odd = even
# But P(A)P(B)P(C)= 1/8
...
# P(B)=0.5
# P(C)=0.5
# P(A)P(B)P(C)=0.5^3=0.125 = P(A*B*C) For noweverything is good but when we do this
# P(A*B)=0.05+0.125 = 0.175 but != P(A)P(B)=0.25
# So the pairs are not independent. Therefore the three events are not mutually independent 
# even though the triple probability equals the product.

#################################### --- c

# n puppies, each independently male/female with probability 1/2.
# P(both sexes)=1-P(all male) - P(all female) = 1-2*2^(-n)
# P(one female)=P(0 female)+P(1 female) = 2^-n + n2^-n
# P(both sexes*one female)= n/2^n
# -> n/2^n = P(both sexes*one female) = P(both sexes)P(one female)=(1-2^1-n)1+n/2^n
# ->1+n=2^n-1
# we need at least 1, so n>=1 -> 1+n=2^2 -> so events are independent only when n=3
...
bin_width <- 0.1
bins <- seq(min(get_prob2_data()), max(get_prob2_data()) + bin_width, bin_width)
hist(get_prob2_data(), breaks=bins, col='orange', freq=TRUE,
     main=paste("Frequency Histogram"),
     xlab="Value", ylab="Frequency")

#################################### --- c
 
# Summary: SO we can see that we have 2 big clusters that is less than 1 year
# and approximately 5 years. We can say that the treatment is working but not for
# everyone. Even so, 5 years is more than average 3, therefore it is worth to try.

#################################### --- d

# Effectivenes: The treatment clearly extends life for some of the patients,
# lots of people surviving at least 5 years. Compared to the 
# nearly universal death by 2.5 years without treatment,this is a strong improvement.
# Limitations: A large number of patients still die within the first year,
# so the treatment can be non effective.
# Recommendations:
# Further research should show us why and how it is so.
# Studies should also look into why some patients benefit signiificantly
# while others do not.
...
# PMF and CDF of Z
outcomes <- expand.grid(X = X_vals, Y = Y_vals)
outcomes$Z <- (outcomes$X + outcomes$Y) / 2
# PMF
pmf_Z <- table(outcomes$Z) / nrow(outcomes)
pmf_Z
# CDF
cdf_Z <- cumsum(pmf_Z)
cdf_Z
...
barplot(pmf_Z, 
        main = "PMF of Z = (X+Y)/2", 
        xlab = "Z values", ylab = "Probability", col = "blue")

# CDF
plot(as.numeric(names(cdf_Z)), cdf_Z, type = "s", lwd = 2,
     main = "CDF of Z", xlab = "Z values", ylab = "Cumulative Probability")
...
# Expected gain
expected_gain <- mean(mapply(gain, outcomes$X, outcomes$Y))
cat("\nExpected gain per game =", expected_gain)
cat("Expected gain after 60 games =", expected_gain * 60)

####################################--- Problem 4 ---#####################################
#################################### --- a

f <- function(h) { 40 - h }   # density function
# https://medium.com/@michael71314/r-lesson-29-an-integral-part-of-r-calculus-f2d0a53d8351
# discovered about integrate func here
N <- integrate(f, lower = 0, upper = 30)$value
cat("Total raisins in box =", N)

#################################### --- b 

g <- function(h) { (40 - h) / N }

curve(g, from = 0, to = 30, 
      main = "PDF of Raisin Height",
      xlab = "Height h (cm)", ylab = "Density",
      col = "blue", lwd = 2)

#################################### --- c 

G <- function(h) { (40*h - h^2/2) / N }

# CDF
curve(G, from = 0, to = 30,
      main = "CDF of Raisin Height",
      xlab = "Height h (cm)", ylab = "Cumulative Probability",
      col = "red", lwd = 2)

#################################### --- d

p_bottom_third <- G(10) - G(0)
cat("Probability raisin is in bottom third =", p_bottom_third)
...
mu <- 2 
sigma <- 3
q1 <- pnorm(mu, mean=mu, sd=sigma)
q2 <- 1 - pnorm(mu + 1.5*sigma, mean=mu, sd=sigma)
q3 <- pnorm(mu + 1.5*sigma, mean=mu, sd=sigma) - 
  pnorm(mu - 1.5*sigma, mean=mu, sd=sigma)

cat("P(X <= mu) =", q1)
cat("P(X - mu > 1.5 sigma) =", q2)
cat("P(|X - mu| < 1.5 sigma) =", q3)


#################################### --- c

lambda <- 2   
F_Y <- function(y, lambda) { 1 - exp(-lambda*y) }
# Y <= 1/lambda
p_exp <- F_Y(1/lambda, lambda)
cat("CDF of Exp(lambda): F(y) = 1 - exp(-lambda*y)")
cat("P(Y <= 1/lambda) =", p_exp)
# Check
cat("Check with R pexp:", pexp(1/lambda, rate=lambda))

####################################--- Problem 6 ---#####################################
#################################### --- a
#ахахаххахпхапхапхпхапхахахахахх хто придумував ці задачі

mu <- 280
sigma <- 8.5
x <- seq(250, 310, by=0.1)
pdf_vals <- dnorm(x, mean=mu, sd=sigma)
cdf_vals <- pnorm(x, mean=mu, sd=sigma)

# PDF
plot(x, pdf_vals, type="l", lwd=2, col="blue",
     main="PDF of Gestation Length",
     xlab="Days", ylab="Density")

# CDF
plot(x, cdf_vals, type="l", lwd=2, col="red",
     main="CDF of Gestation Length",
     xlab="Days", ylab="Cumulative Probability")

#################################### --- b

p_before_exam <- pnorm(273, mean=mu, sd=sigma)
cat("P(birth <= May 18) =", p_before_exam)

#################################### --- c

p_may_after_exam <- pnorm(286, mu, sigma) - pnorm(274, mu, sigma)
cat("P(birth in May 19-31) =", p_may_after_exam)

#################################### --- d

# P(X <= d) = 0.05
d <- qnorm(0.05, mean = mu, sd = sigma)

days_before_due <- mu - d 
days_before_due
rounded_days <- round(days_before_due)
# тутможемо підставити що хочемо
due_date <- as.Date("2025-9-29")
exam_date <- due_date - rounded_days

cat("d (gestation day cutoff) =", d)
cat("Days before due (raw) =", days_before_due)
cat("Shift days (rounded) =", rounded_days)
cat("Due date =", format(due_date, "%Y-%m-%d"))
cat("Exam date should be around:", format(exam_date, "%Y-%m-%d"))
